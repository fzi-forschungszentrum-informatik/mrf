...
//    tf.translation() = Eigen::Vector3d(5, 0, -8);
    ...
    cl->push_back(pcl::transformPointWithNormal(p, tf.cast<float>()));

    p.getVector3fMap() = Eigen::Vector3f(1, rows - 1, 1);
    p.getNormalVector3fMap() = Eigen::Vector3f(0, 0, -1);
    cl->push_back(pcl::transformPoint(p, tf.cast<float>()));
...
TEST(Solver, Solve) {
    ...
    constexpr size_t rows = 80;
    constexpr size_t cols = 120;
