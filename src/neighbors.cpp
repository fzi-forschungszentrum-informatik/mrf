...
std::vector<Pixel> getNeighbors(const Pixel& p,
    ...
    if (p.row > 0)
        neighbors.emplace_back(p.col, p.row - 1, img.at<float>(p.row - 1, p.col));
    if (p.col > 0)
        neighbors.emplace_back(p.col - 1, p.row, img.at<float>(p.row, p.col - 1));
    ...
    if (static_cast<int>(mode) > 4) {
        if (p.row > 0 && p.col < img.cols - 1)
            neighbors.emplace_back(p.col + 1, p.row - 1, img.at<float>(p.row - 1, p.col + 1));
